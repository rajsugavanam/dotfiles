...
git_bubble () {
    ...
    if [[ -n $git_branch ]]; then
		...
		# local gitst= $(echo -n $(git_super_status))
		# echo -n "$(bubblify 3 $gitst $git_color $bubble_color) "
		# echo -n $gitst
    fi
}
...
PROMPT='$(ssh_bubble)$user_machine_bubble$filepath_bubble$(git_bubble)$_newline$end_of_prompt%{$reset_color%}'
RPROMPT=''
